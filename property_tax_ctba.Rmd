...
```{r, warning=FALSE, error=FALSE}
census_data_18 <- left_join(census_data, racedata, by=c ("GEOID", "NAME"))
```
...
movers <- movers %>%
  rename(movers_18 = estimate)
```

```{r, warning=FALSE, error=FALSE}

movers <- st_transform(movers, "+proj=longlat +datum=NAD83 +no_defs")

```

```{r}
movers$inter <- as.numeric(st_intersects(movers, cityboundary))

movers <- movers %>% filter(inter == 1)
```
