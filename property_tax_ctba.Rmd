...
quantile(gentrification, c(.90), na.rm = T) 
```
...
census <- census %>%
  mutate(one_gentri_3 = case_when(
    gentri_3 >= 0.1143626 & household_income_13 < 57272.42 ~ 1,
    household_income_13 > 57272.42 ~ 0,
    gentri_3 < 0.1143626 ~ 0
    )
  )
```
...
This measure of gentrification reaches 26 tracts.
...
leaflet(options = leafletOptions(zoomSnap = 0.1)) %>%
  addTiles() %>%
  addPolygons(
    data = st_geometry(census %>% filter(census$one_gentri_3==1)),
    color = "black",
    fillColor = color_200k(census$gentri_3),
    weight = 2.5,
    fillOpacity = .3,
    smoothFactor = .2,
    popup = tract_labels,
...
What are the correlations between the different definitions of gentrificaion?

```{r, warning=FALSE, error=FALSE}

cor(census$gentri_1, census$gentri_2, use = "complete.obs")

cor(census$gentri_2, census$one_gentri_3, use = "complete.obs")

cor(census$gentri_1, census$one_gentri_3, use = "complete.obs")
