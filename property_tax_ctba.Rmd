...
builtdata <- builtdata %>%
  rename(
    built_14_18 = B25034_002,
    built_10_13_18 = B25034_003,
    built_00_09_18 = B25034_004,
    built_90_99_18 = B25034_005,
    built_80_89_18 = B25034_006,
    built_70_79_18 = B25034_007,
    built_60_69_18 = B25034_008,
    built_50_59_18 = B25034_009,
    built_40_49_18 = B25034_010,
    built_39_18 = B25034_011
  )
```
...
census <- census %>%
  mutate(share_white_18 = white_18 / sum_18)

census <- census %>%
  mutate(share_black_18 = black_18 / sum_18)

census <- census %>%
  mutate(share_latino_18 = latino_18 / sum_18)

census <- census %>%
  mutate(share_white_only_18 = white_not_latino_18 / sum_18)

census <- census %>%
  mutate(share_white_13 = white_13 / sum_13)

census <- census %>%
  mutate(share_black_13 = black_13 / sum_13)

census <- census %>%
  mutate(share_latino_13 = latino_13 / sum_13)

census <- census %>%
  mutate(share_white_only_13 = white_not_latino_13 / sum_13)
...
census <- census %>%
  mutate(increase_income = (household_income_18 / household_income_13 - 1) * 100)

census <- census %>%
  mutate(increase_income_pc = (income_pc_18 / income_pc_13 - 1) * 100)

census <- census %>%
  mutate(increase_pop = (population_18 / population_13 - 1) * 100)

census <- census %>%
  mutate(increase_rent = (median_rent_18 / median_rent_13 - 1) * 100)

census <- census %>%
  mutate(increase_latino = (share_latino_18 / share_latino_13-1)*100)

census <- census %>%
  mutate(increase_black = (share_black_18 / share_black_13-1)*100)

is.na(census$increase_latino) <- sapply(census$increase_latino, is.infinite)

is.na(census$increase_black) <- sapply(census$increase_black, is.infinite)

census$increase_black[is.nan(census$increase_black)] <- NA

census$increase_latino[is.nan(census$increase_latino)] <- NA
```

Change in share black and latino:
