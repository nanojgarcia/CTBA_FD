...
census <- census %>%
  mutate(gentri_4 = (white_18  - white_13) / sum_13)
...
census <- census %>%
  mutate(gentri_5 = (white_18/white_13) / (sum_18/sum_13))
...
ggplot(census) +
  ...
  geom_vline(xintercept = 0.01232853, color="blue") +
  ggtitle("Histogram price increase in home values")
...
census <- census %>%
  mutate(one_gentri_4 = case_when(
    gentri_4 >= 0.15172783 & household_income_13 < 58676 ~ 1,
    household_income_13 > 58676 ~ 0,
    gentri_4 < 0.15172783 ~ 0
    )
  )
```
...
Definition number 4 reaches 23 tracts.
...
census <- census %>%
  mutate(gentri_5 = ((white_18/white_13) / (sum_18/sum_13)-1)*100)
...
census %>%
  summarise(median_gentri_5=median(gentri_5, na.rm=T))

ggplot(census) +
  geom_histogram(aes(x = gentri_5, y = ..density..),
                 binwidth = .5,
    colour = "black", fill = "white"
  )  +
  geom_vline(xintercept = 2.077133, color="blue") +
  ggtitle("Histogram price increase in home values")
...
```{r, warning=FALSE, error=FALSE}
gentrification = census$gentri_5
